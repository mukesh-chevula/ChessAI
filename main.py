...
def is_valid_pawn_move(start, end, color):
    ...
    if sc == ec:
        if color == "w" and sr == 6 and er == sr - 2 and not board[er][ec]:
            return True
        elif color == "b" and sr == 1 and er == sr + 2 and not board[er][ec]:
            return True
        elif abs(er - sr) == 1 and not board[er][ec]:
            return True
    elif abs(sc - ec) == 1 and abs(er - sr) == 1 and board[er][ec] and board[er][ec][0] != color:
        return True
    return False
...
def is_valid_bishop_move(start, end):
    ...
    if abs(sr - er) == abs(sc - ec):
        return not is_blocked(start, end)
    return False
